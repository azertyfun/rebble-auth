...
func AccountLoginHandler(ctx *HandlerContext, w http.ResponseWriter, r *http.Request) (int, error) {
	...
	accessToken, userErr, err := ctx.Database.AccountLoginOrRegister(sso.Name, claims["sub"].(string), name, tokensStatus.AccessToken, int64(claims["exp"].(float64)), r.RemoteAddr)
	if err != nil {
...
func AccountInfoHandler(ctx *HandlerContext, w http.ResponseWriter, r *http.Request) (int, error) {
	...
	loggedIn, errorMessage, err := ctx.Database.SessionInformation(auth.AccessToken)
	if err != nil {
		return http.StatusInternalServerError, err
	}
	...
	loggedIn, name, err := ctx.Database.AccountInformation(auth.AccessToken)
	if err != nil {
		return http.StatusInternalServerError, err
	}
...
func AccountUpdateNameHandler(ctx *HandlerContext, w http.ResponseWriter, r *http.Request) (int, error) {
	...
	errorMessage, err = ctx.Database.UpdateName(auth.AccessToken, auth.Name)
