...
// Returns success, errorMessage, name, linkedProviders, err
// err is only returned if the error was unexpected (internal server error vs bad request)
func Info(database *db.Handler, accessToken string) (bool, string, string, []string, error) {
	...
	loggedIn, name, linkedProviders, err := database.AccountInformation(accessToken)
	if err != nil {
		return false, "Internal Server Error: Could not query account information from database", "", []string{}, err
	}

	return loggedIn, "", name, linkedProviders, nil
}
